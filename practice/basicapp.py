...
client = AzureOpenAI(
    api_key="4076f9d9575544e384c754dd47ecf804", 
...
# Function to add new input and generate SQL
def process_input(new_input):
    # Add previous inputs to the context
    context = " ".join(conversation_history)  # Merge past inputs into context
    full_input = context + " " + new_input  # Combine with new input
    
    # Pass the combined input to the LLM for SQL generation
    # sql_query = generate_sql_from_llm(full_input)
    
    # Update the conversation history
    conversation_history.append(new_input)
    
    return full_input
# Example LLM SQL generation function
#def generate_sql_from_llm(input_text):
    # Your LLM integration logic here
 #   return f"Generated SQL from: {input_text}"
# Example usage
#new_query = "Get the total sales from the 'Orders' table"
#sql = process_input(new_query)
#print(sql)
